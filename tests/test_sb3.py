...
from mlflow_rl_tools import sb3

# Before running the tests, make sure MLflow server is running with the following command:
# mlflow server --port 5000

REGISTERED_MODEL_NAME = "test_sb3_log_model"
REGISTERED_MODEL_NAME_VEC = "test_sb3_log_model_vec"
...
def setup_module(module):
    ...
    mlflow.set_experiment("test_sb3")


@pytest.mark.parametrize("model, model_name", zip(models, registered_model_names))
def test_sb3_log_model(model: PPO, model_name: str):
    with mlflow.start_run():
        model_info: ModelInfo = sb3.log_model(
...
def test_sb3_load_model(model_name: str, env: gym.Env):
    model: SelfBaseAlgorithm = sb3.load_model(f"models:/{model_name}/latest", env=env)
